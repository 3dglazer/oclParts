...
//float4 testingTurbulence(float4 position){
//	//float pos=ugnoise3dOctaves(position,5,0.5,3.0,1.0);
//	return sampleGradientNoiseVectorField(position,3,0.5,3.0,0.5);
//	//return (float4) (pos,pos,pos,0.0);
//}
...
float4 fd_drag(float4 particlePosition,float4 particleVelocity,__constant Fd_drag* fd, int fdNum){
	...
	float4 force=normalize(particleVelocity)*magnitude;
...
float4 fd_turbulence(float4 particlePosition,__constant Fd_turbulence* fd, int fdNum){
	...
	float4 vc;
	vc= normalize(sampleGradientNoiseVectorField(vec4_add3(particlePosition,phase), 1,fd[fdNum].noiseRatio,2., 1.0)); //bylo to 0.3,1.0))
	//float4 koef=((float)sign(vc.x),(float)sign(vc.y),(float)sign(vc.z),0.0);
	//vc= normalize(sampleGradientNoiseVectorField(vec4_add3(particlePosition,phase), 0.5,noiseRatio,frequency, 10.0));
	return (float4)vc*magnitude;
}
////========================== /Force calculations ===========================
...
//	
	...
	v=normalize(v);
	float ratio=clamp((0.5f-(0.5f-life)*.05f)*2.0f,0.0f,1.0f);
	//ratio*=ratio;
	float rs=1.0-ratio;
	col=(col*ratio)+(vec4_add3(v,(float4)(0.5,0.5,0.5,0.5))*0.5)*rs;
    	color[i]=clamp(col,0.0f,1.0f);

	//color[i]=(float4)(1.0,1.0,1.0,1.0)*0.1;
	color[i].w=2.0*ratio;
}
